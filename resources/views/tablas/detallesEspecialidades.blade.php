        ...
        <div class="card">
            <div class="card-body">
                <div class="table-responsive">
                    ...
                    <table style="width: 100% !important" id="especialidadesTable"
                        class="example table table-bordered table-striped">
                        <thead>
                            <tr>
                                <th style="text-align: left;">Nombre de la especialidad</th>
                        ...
                        <tbody>
                            @foreach($especialidades as $especialidad)
                                <tr>
                                    <td style="text-align: left;">{{ $especialidad->descripcion }}</td>
                                    <td>
                                        <button
                                            class="btn {{ $especialidad->estatus == 1 ? 'btn-success' : 'btn-danger' }} btn-sm"
                                            onclick="toggleStatus({{ $especialidad->id }}, {{ $especialidad->estatus }})">
...
<script>
    new DataTable('#especialidadesTable', {
        pageLength: 10,
        layout: {
            topStart: [
                'buttons',
...
<script>
    function toggleStatus(id, currentStatus) {
        ...
        fetch(`/especialidades/${id}/status`, {
            method: 'PATCH',
            headers: {
