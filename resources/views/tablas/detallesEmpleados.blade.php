<x-app-layout>
    ...
    <div class="d-flex justify-content-end mr-5">
        ...
        <a class="btn btn-success text-white my-2" data-toggle="modal" data-target="#addEmpleado">Añadir</a>
        @include('tablas.agregarEmpleado')
    </div>
        ...
        <div class="card">
            <div class="card-body">
                <div class="table-responsive">
                    ...
                    <table style="width: 100% !important" id="empleadosTable"
                        ...
                        <thead>
                            <tr>
                                ...
                                <th style="text-align: left;">Nombre de Unidad</th>
                                <th style="text-align: left;">Código Trabajador</th>
                        ...
                        <tbody>
                            @foreach($empleados as $empleado)
                                @php
                                    $fechaNacimiento = new DateTime($empleado->pacientes->personas->fecha_nacimiento);
                                    $fechaActual = new DateTime();
                                    $edad = $fechaActual->diff($fechaNacimiento)->y;
                                @endphp
                                <tr>
                                    <td style="text-align: left;">{{ $empleado->pacientes->personas->cedula }}</td>
                                    <td style="text-align: left;">{{ $empleado->pacientes->personas->nombres }}
                                        {{ $empleado->pacientes->personas->apellidos }}</td>
                                    <td style="text-align: left;">{{ $edad }}</td>
                                    <td style="text-align: left;">{{ $empleado->nombre_unidad }}</td>
                                    <td style="text-align: left;">{{ $empleado->codigoTrabajador }}</td>
                                    <td>
                                        <button
                                            class="btn {{ $empleado->estatus == 1 ? 'btn-success' : 'btn-danger' }} btn-sm"
                                            onclick="toggleStatus({{ $empleado->id }}, {{ $empleado->estatus }})">
